...
// Warhammer 40K пасхалка - автовоспроизведение видео
function initWarhammerVideo() {
    const warhammerTrigger = document.getElementById('warhammerTrigger');
    const warhammerModal = document.getElementById('warhammerModal');
    const warhammerVideo = document.getElementById('warhammerVideo');
    const closeModal = warhammerModal.querySelector('.close-modal');
    const imperiumButton = document.getElementById('imperiumButton');

    if (warhammerTrigger && warhammerModal && warhammerVideo) {
        // Показываем модальное окно при клике на триггер
        warhammerTrigger.addEventListener('click', function() {
            warhammerModal.classList.add('active');
            document.body.style.overflow = 'hidden';
            
            // Автоматически запускаем видео после небольшой задержки
            setTimeout(() => {
                warhammerVideo.currentTime = 0; // Перематываем в начало
                warhammerVideo.play().catch(error => {
        ...
        function showVideoPlayButton() {
            ...
            playButton.style.background = 'linear-gradient(135deg, #8B0000, #B22222)';
            ...
            warhammerVideo.parentNode.insertBefore(playButton, warhammerVideo.nextSibling);
        }

        // Обработчики для видео
        warhammerVideo.addEventListener('ended', function() {
            // Повторяем видео при завершении
            this.currentTime = 0;
            this.play();
        });

        warhammerVideo.addEventListener('pause', function() {
            // Автоматически продолжаем воспроизведение при паузе
            setTimeout(() => {
                if (!this.ended) {
                    this.play();
                }
        ...
        closeModal.addEventListener('click', function() {
            warhammerModal.classList.remove('active');
            document.body.style.overflow = 'auto';
            warhammerVideo.pause();
            warhammerVideo.currentTime = 0;
        });

        // Кнопка "За Империум!"
        if (imperiumButton) {
            imperiumButton.addEventListener('click', function() {
                showNotification('За Империум Человечества! 🛡️');
                warhammerModal.classList.remove('active');
                document.body.style.overflow = 'auto';
                warhammerVideo.pause();
                warhammerVideo.currentTime = 0;
        ...
        warhammerModal.addEventListener('click', function(e) {
            if (e.target === this) {
    ...
    Object.entries(secretCombos).forEach(([game, combo]) => {
        if (keySequence.slice(-combo.length).join('') === combo.join('')) {
            if (game === 'warhammer') {
                ...
                if (warhammerModal) {
                    warhammerModal.classList.add('active');
                    showNotification('Warhammer 40K активирован! ⚔️');
                }
            } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Инициализируем Warhammer с видео
    initWarhammerVideo();
    ...
    if (logo) {
        ...
        // Warhammer пасхалка в логотипе (двойной клик)
        let clickCount = 0;
        let clickTimer;
        ...
        logo.addEventListener('click', () => {
            ...
            if (clickCount === 1) {
                ...
                if (warhammerModal) {
                    warhammerModal.classList.add('active');
                    showNotification('Император защищает! 👑');
                }
                clickCount = 0;
            }
        });
